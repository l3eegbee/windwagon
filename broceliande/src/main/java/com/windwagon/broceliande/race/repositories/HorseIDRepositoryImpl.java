...
public class HorseIDRepositoryImpl implements HorseIDRepositoryCustom {
	...
	public HorseID findFather(HorseID child) {
		return horseIDRepository.findFirstByNameAndSexAndBirthBefore(child.getFather(), Sex.MALE, Date.from(child.getBirth().toInstant().minus(birthDelta, ChronoUnit.YEARS)));
	}

	@Override
	public HorseID findMother(HorseID child) {
		return horseIDRepository.findFirstByNameAndSexAndBirthBefore(child.getMother(), Sex.FEMALE, Date.from(child.getBirth().toInstant().minus(birthDelta, ChronoUnit.YEARS)));
	}
