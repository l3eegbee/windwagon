	...
	public synchronized Meeting loadMeeting(JsonNode meetingNode) throws PMUError {
		...
		try {
			...
			if (!rcNameNode.isMissingNode() && !rcCountryNode.isMissingNode()) {
				...
				Racecourse racecourse = racecourseRepository.findByNameAndCountry(rcName, rcCountry);
				if (racecourse == null) {
			...
			if (!windSpeedNode.isMissingNode()) meeting.setWindSpeed(windSpeedNode.asDouble());

			// wind direction
			JsonNode windDirectionNode = meetingNode.at("/meteo/directionVent");
			if (!windDirectionNode.isMissingNode()) {
				meeting.setWindDirection(pmuUtils.findPMU(WindDirection.class, windDirectionNode));
			} else {
			...
			if (!climatNode.isMissingNode()) meeting.setClimat(pmuUtils.asText(climatNode));
