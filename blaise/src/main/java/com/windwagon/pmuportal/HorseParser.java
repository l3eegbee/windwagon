...
public class HorseParser {
	...
	public synchronized Horse parseHorse(Race race, JsonNode horseNode) throws PMUError {
		...
		if (race.getId() != null) horse = horseRepository.findByRaceAndNumber(race, hnum);
		...
		try {
			...
			HorseID horseID = horseIDRepository.findByNameAndBirthAndSexAndFatherAndMother(name, birth, mfSex, father, mother);

			if (horseID == null) {

				HorseID switchedHorsedID = horseIDRepository.findByNameAndBirthAndSexAndFatherAndMother(name, birth, mfSex, mother, father);
			...
			if (sex == Sex.GELDING) {
				...
				if (gelding == null || gelding.after(now)) horseID.setGelding(now); // MAZEL TOV!
			}
			...
			if (!breedNode.isMissingNode()) {
				...
				if (horseBreed == null || breed != Breed.INCONNU) horseID.setBreed(breed);
			}
			...
			if (!coatNode.isMissingNode()) {
				...
				if (horseCoat == null || coat != Coat.INCONNU) horseID.setCoat(coat);
			}
			...
			if (!stallNode.isMissingNode()) horse.setStall(stallNode.asInt());

			// stable
			JsonNode stableNode = horseNode.at("/ecurie");
			if (!stableNode.isMissingNode()) horse.setStable(pmuUtils.asText(stableNode));

			// weight
			JsonNode weightNode = horseNode.at("/handicapPoids");
			if (!weightNode.isMissingNode()) horse.setWeight(weightNode.asDouble() / 10L);
			...
			JsonNode previousYearProfitNode = horseNode.at("/gainsParticipant/gainsAnneePrecedente");
			...
			if (!yearProfitNode.isMissingNode()) horse.setYearProfit(yearProfitNode.asDouble());
			...
			if (!placeProfitNode.isMissingNode()) horse.setPlaceProfit(placeProfitNode.asDouble());
