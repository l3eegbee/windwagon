...
public class $className extends MeetingBuilder {
    ...
    public static Meeting build() {
        ...
        // no racecourse
#end
        meeting.setNumber( $meeting.getNumber() );
#if( $null.isNotNull( $meeting.getWindSpeed() ) )
        meeting.setWindSpeed( $meeting.getWindSpeed() );
#else
        // no wind speed
#end
        ...
        // no wind direction
#end
#if( $null.isNotNull( $meeting.getTemperature() ) )
        meeting.setTemperature( $meeting.getTemperature() );
#else
        // no temperature
#end
#if( $null.isNotNull( $meeting.getClimat() ) )
        meeting.setClimat( "$meeting.getClimat()" );
#else
        // no climat
#end
#if( $null.isNotNull( $meeting.getField() ) )
        meeting.setField( Field.$meeting.getField() );
#else
        // no field
#end
...
#foreach( $race in $sorter.sortSet( $meeting.getRaces() ) )
        races.add( #raceBuilderName( $race )( meeting ) );
#end
...
#foreach( $race in $sorter.sortSet( $meeting.getRaces() ) )
    ...
    public static Race #raceBuilderName( $race )( Meeting meeting ) {
        ...
        // no real start
#end
#if( $null.isNotNull( $race.getDistance() ) )
        race.setDistance( $race.getDistance() );
#else
        // no distance
#end
#if( $null.isNotNull( $race.getBenefit() ) )
        race.setBenefit( $race.getBenefit() );
#else
        // no benefit
#end
#if( $null.isNotNull( $race.getDiscipline() ) )
        race.setDiscipline( Discipline.$race.getDiscipline() );
#else
        // no discipline
#end
...
#foreach( $horse in $sorter.sortSet( $race.getHorses() ) )
        horses.add( #horseBuilderName( $horse )( race ) );
#end
...
#foreach( $odds in $sorter.sortSet( $race.getOdds() ) )
        ...
        // no odds
#end
#if( $null.isNotNull( $race.getDuration() ) )
        race.setDuration( $race.getDuration() );
#else
        // no duration
#end
...
#foreach( $place in $sorter.sortSet( $race.getPlaces() ) )
        places.add( place( race, #alignNum( $place.getNumber() ), #alignNum( $place.getHorse() )#if( $place.getDistance() ), "$place.getDistance()"#end#if( $place.getDuration() ), $place.getDuration()#end ) );
#end
        ...
        // no finish
#end
#if( $null.isNotNull( $race.getStopped() ) )
        race.setStopped( "$race.getStopped()" );
#else
        // no stopped
#end
#if( $null.isNotNull( $race.getDisqualified() ) )
        race.setDisqualified( "$race.getDisqualified()" );
#else
        // no disqualified
#end
#if( $null.isNotNull( $race.getFallen() ) )
        race.setFallen( "$race.getFallen()" );
#else
        // no fallen
#end
...
#horseIDBuilder( $horse $horse.getHorseID() "" )
    ...
    public static Horse #horseBuilderName( $horse )( Race race ) {

        Horse horse = new Horse();

        horse.setRace( race );
        horse.setHorseID( #horseBuilderName( $horse )_ID() );
        horse.setNumber( $horse.getNumber() );
        horse.setRunner( $horse.getRunner() );
#if( $null.isNotNull( $horse.getSupplemented() ) )
        horse.setSupplemented( $horse.getSupplemented() );
#else
        // no supplemented value
#end
#if( $null.isNotNull( $horse.getPregnant() ) )
        horse.setPregnant( $horse.getPregnant() );
#else
        // no pregnant value
#end
#if( $null.isNotNull( $horse.getStall() ) )
        horse.setStall( $horse.getStall() );
#else
        // no stall
#end
#if( $null.isNotNull( $horse.getStable() ) )
        horse.setStable( "$horse.getStable()" );
#else
        // no stable
#end
#if( $null.isNotNull( $horse.getWeight() ) )
        horse.setWeight( $horse.getWeight() );
#else
        // no weight
#end
#if( $null.isNotNull( $horse.getDistance() ) )
        horse.setDistance( $horse.getDistance() );
#else
        // no distance
#end
#if( $null.isNotNull( $horse.getCareerProfit() ) )
        horse.setCareerProfit( $horse.getCareerProfit() );
#else
        // no career profit
#end
...
#end
#if( $null.isNotNull( $horse.getVictoryProfit() ) )
        horse.setVictoryProfit( $horse.getVictoryProfit() );
#else
        // no victory profit
#end
#if( $null.isNotNull( $horse.getPlaceProfit() ) )
        horse.setPlaceProfit( $horse.getPlaceProfit() );
#else
        // no place profit
#end
#if( $null.isNotNull( $horse.getYearProfit() ) )
        horse.setYearProfit( $horse.getYearProfit() );
#else
        // no year profit
#end
#if( $null.isNotNull( $horse.getHorseshoe() ) )
        horse.setHorseshoe( Horseshoe.$horse.getHorseshoe() );
#else
        // no horseshoe
#end
#if( $null.isNotNull( $horse.getBlinder() ) )
        horse.setBlinder( Blinder.$horse.getBlinder() );
#else
        // no blinder
#end
        ...
        // no trainer
#end
#if( $null.isNotNull( $horse.getOwner() ) )
        horse.setOwner( owner( "$horse.getOwner().getName()" ) );
#else
        // no owner
#end
        ...
        // no rancher
#end
#if( $null.isNotNull( $horse.getJockey() ) )
        horse.setJockey( jockey( "$horse.getJockey().getName()" ) );
#else
        // no jockey
#end
...
#set( $oddsNumber = $velocityCount )
    ...
    public static Odds #oddsBuilderName( $odds $oddsNumber )( Race race ) {

        Odds odds = new Odds();

        odds.setRace( race );
        odds.setDate( parse( "$dateFormat.format($odds.getDate())" ) );
#if( $null.isNotNull( $odds.getTotal() ) )
        odds.setTotal( $odds.getTotal() );
#else
        // no total
#end
