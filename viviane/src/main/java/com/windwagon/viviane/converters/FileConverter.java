...
public class FileConverter implements Converter<File> {
	...
	@Override
	public boolean getAllPossibleValues(List<Completion> completions, Class<?> requiredType, String userInput, String optionContext, MethodTarget target) {
		...
		if (userFile.exists() && userFile.isDirectory()) {
			...
			if (parent == null) parent = userFile;
	...
	private String expand(String path) {
		...
		if (path.startsWith(HOME_SYMBOL)) path = path.replaceFirst(HOME_SYMBOL, HOME);
	...
	private List<File> getFilterFiles(File dir, String prefix, List<String> exts) {
		...
		for (File file : listFiles)
			if (match(file, prefix, exts)) files.add(file);

		if (files.size() == 1) {
			File file = files.get(0);
			if (file.isDirectory()) return getFilterFiles(file, "", exts);
		}
	...
	private boolean match(File file, String prefix, List<String> exts) {
		...
		if (file.isDirectory() || exts.isEmpty()) return true;
