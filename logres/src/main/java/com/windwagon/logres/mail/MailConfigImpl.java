...
public class MailConfigImpl implements MailConfig {
	...
	public void init() {
		...
		enabled = propertyResolver.getProperty(MAIL_ENABLED, Boolean.class, defaultConfig.getEnabled());
		if (!enabled) return;
		...
		mailSubject = templatize(TEMPLATE_NAME_SUBJECT, prop(MAIL_SUBJECT, defaultConfig.getSubject()));
