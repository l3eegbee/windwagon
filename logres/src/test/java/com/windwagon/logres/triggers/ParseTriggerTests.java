...
@RunWith(SpringRunner.class) //v2
@SpringBootTest(classes = TriggerTestsConfig.class) //v2
//@SpringApplicationConfiguration(classes = TriggerTestsConfig.class) //v1
...
public class ParseTriggerTests {
	...
	public void parseOffsets() {
		...
		trigger.setOffsets(appCxt.getBean(Offset.class, Duration.of(1, ChronoUnit.HOURS), false));
		assertThat(triggerParser.createOffsetTrigger("+1h"), is(trigger));

		ChronoUnit H = ChronoUnit.HOURS;
		ChronoUnit M = ChronoUnit.MINUTES;

		trigger.setOffsets(appCxt.getBean(Offset.class, Duration.of(-2, H), false), appCxt.getBean(Offset.class, Duration.of(-5, M), false), appCxt.getBean(Offset.class, Duration.of(0, M), false), appCxt.getBean(Offset.class, Duration.of(2, M), true));
